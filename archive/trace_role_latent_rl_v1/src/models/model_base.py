...
class LitCoTModelBase(pl.LightningModule):
    def __init__(
        ...
        llm_path = model_kwargs.get("llm_path")
        if llm_path is None:
            llm_path = opj(all_config.args.workspace_path, "models", "llms", model_kwargs.model_id)
        if not os.path.exists(llm_path):
            raise FileNotFoundError(
                f"Cannot find the base model at {llm_path}. "
                "Please either set --workspace_path to the correct root, or pass model.model_kwargs.llm_path=/abs/path/to/your/model."
            )
...
"""
        ...
        if torch_dtype:
            if torch_dtype == "auto":
                ...
                model_load_kwargs["dtype"] = getattr(torch, str(torch_dtype))
        if model_kwargs.get("attn_implementation") is not None:
            model_load_kwargs["attn_implementation"] = model_kwargs.attn_implementation

        self.llm: LlamaForCausalLM = AutoModelForCausalLM.from_pretrained(llm_path, **model_load_kwargs)
        self.is_vl_model = False
        self.hidden_size = getattr(self.llm.config, "hidden_size", None)
        if self.hidden_size is None and getattr(self.llm.config, "text_config", None) is not None:
            self.hidden_size = getattr(self.llm.config.text_config, "hidden_size", None)
        if self.hidden_size is None:
            raise ValueError(f"Cannot infer hidden size from config at {llm_path}")
    ...
    def make_position_ids_for_current_input(self, attention_mask: torch.Tensor, current_length: int):
        """Return optional position ids for models that need current-block ids."""
        if not getattr(self, "is_vl_model", False):
            return None
        positions = attention_mask.long().cumsum(dim=-1) - 1
        positions = positions.masked_fill(attention_mask == 0, 0)
        current_positions = positions[:, -current_length:]
        return current_positions.unsqueeze(0).expand(4, -1, -1)
    ...
    def on_test_start(self):
        ...
        dataset_name = getattr(self.all_config.data_module, "dataset_name", "unknown")
        safe_dataset_name = "".join(ch if ch.isalnum() or ch in {"-", "_"} else "_" for ch in str(dataset_name))
        self.json_logger = JsonLogger(self, log_file_name=f"test_{get_timestamp()}_{safe_dataset_name}_pid{os.getpid()}")
        return super().on_test_start()

    def test_step(self, batch, batch_idx=None, dataloader_idx=0):
        if batch_idx is not None and batch_idx % 100 == 0:
            dataset_name = getattr(self.all_config.data_module, "dataset_name", "unknown")
            self.text_logger.log(f"Test progress: dataset={dataset_name}, batch_idx={batch_idx}")
    ...
    def latent_generate(
        ...
        min_n_latent_forward = int(latent_generation_config.get("min_n_latent_forward", 0) or 0)
        min_n_latent_forward = max(0, min(min_n_latent_forward, int(max_n_latent_forward)))
        ...
        for _ in range(max_n_latent_forward):
            ...
            if min_n_latent_forward > 0:
                is_eol = is_eol & (n_latent_forward >= min_n_latent_forward)
    ...
    def eval_generation(self, batch, split="val", batch_idx=None, dataloader_idx=0):
        ...
        elif sft_method == "trace_trajectory":
            outputs_token_ids, n_latent_forward = self.trace_latent_generate(questions=questions)
