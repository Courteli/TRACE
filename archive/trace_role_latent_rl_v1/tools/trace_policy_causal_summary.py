...
---------------
Core conclusion:
    Answer behavior should depend on the ordered PLAN/SOLVE/CHECK computation
    delivered through deterministic COMMIT. Replacing a stochastic transition
    should reduce support after suffix recomputation, while masking COMMIT
    should test the final delivery channel.
Evidence logic:
    ...
    preserve action norm. PLAN through CHECK interventions keep the original
    ...
    the COMMIT readout instead of inventing a random COMMIT action.
Review risks:
    The competitive decoder can reread the question. These interventions test
    whether path content adds behaviorally relevant information beyond that
    direct context; they do not establish exclusive architectural mediation or
    a unique human-readable interpretation for any latent state.
"""
...
def plot_prefix(prefix_score: dict, prefix_accuracy: dict, output_base: Path):
    ...
    for ax, values, ylabel, scale in (
        ...
        ax.set_xticks(steps)
        ax.set_xlabel("Requested prefix (COMMIT-only readout)")
...
def main():
    ...
    checks = {
        "behavioral_no_path_dependence_given_question_context": (
        ...
        "full_path_adds_information_over_question_only": (
    ...
    report = {
        ...
        * 7
        ...
        "claim_boundary": (
            "The main decoder also reads the question. Passing interventions "
            "supports incremental behavioral use of the path, not exclusive "
            "architectural mediation. A failed transition gate narrows the "
            "claim and cannot be replaced with a selected 3D view. The "
            "all-position gate uses family-wise 95% Bonferroni bootstrap "
            "intervals over seven stochastic roles plus deterministic COMMIT."
        ),
        "gold_score_protocol": (
            "frozen Stage-1 question-conditioned direct-answer scorer; "
            "question fixed within each path intervention"
        ),
    }
    ...
    contract = {
        ...
        "controls": [
            ...
            "same-norm PLAN-through-CHECK replacement with suffix recomputation",
