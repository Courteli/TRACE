...
---------------
Core conclusion:
    The fixed PLAN--SOLVE--CHECK latent roles align with frozen textual-CoT
    teacher semantics while retaining role-conditioned exploration, and the
    final COMMIT transition is conditionally deterministic and is the only
    latent state exposed to answer generation.
Evidence chain:
...
def role_question_metrics(record: dict) -> Dict[str, float]:
    ...
    semantic_groups = {
        ...
        "check": np.asarray([6], dtype=np.int64),
    }
    exploration_groups = {
        "plan": np.asarray([0], dtype=np.int64),
...
def plot_step_profile(
    ...
    ax.set_xticks(steps, ("P", "S1", "S2", "S3", "S4", "S5", "C", "D"))
    second = ax.twinx()
    second.plot(
        steps,
        step_norm,
...
def main():
    ...
    metrics = {
        "evidence_priority": {
            "primary": (
                "PLAN/SOLVE/CHECK frozen-text-CoT teacher similarity"
            ),
            "supporting": (
            ...
            "diagnostic_only": (
                "legacy corridor and answer-outcome path geometry"
            ),
    ...
    contract = {
        "core_conclusion": (
            "PLAN, masked SOLVE, and CHECK align to frozen textual-CoT "
            "teacher semantics; their role-conditioned policies retain "
            "measurable exploration; COMMIT is conditionally deterministic "
            "and is the answer generator's only latent readout."
        ),
        "primary_evidence": (
            "question-bootstrap teacher cosine similarity for PLAN/SOLVE/"
            "CHECK, role Gaussian std/entropy, and a numerical COMMIT audit"
        ),
