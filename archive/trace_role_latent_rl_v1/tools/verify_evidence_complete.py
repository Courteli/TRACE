...
def validate(root: Path, stage1: Path, final: Path) -> dict:
    ...
    if comparison.get("evidence_priority", {}).get("primary") != "role_semantics":
        raise SystemExit("stage comparison does not prioritize role semantics")
    ...
    for relative in (
        ...
        "prefix_sufficiency.svg",
        "prefix_sufficiency.pdf",
        "prefix_sufficiency.tiff",
