...
step_level_supervision=PLAN_forecast_plus_SOLVE1_5_text_decode_plus_REFINE_endpoint
...
for target_max_epochs in $(seq 1 10); do
  ...
  while true; do
    ...
    "${TRACE_VB_PYTHON}" run.py \
      ...
      model.model_kwargs.trace_policy_config.stage1_solve_text_weight=0.20 \
...
if failures:
    raise SystemExit("TRACE-VB-v3 Epoch-1 behavior gate failed: " + "; ".join(failures))
PY
  fi
done
