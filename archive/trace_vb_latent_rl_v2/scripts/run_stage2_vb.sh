...
if checkpoint.get("trace_vb_schema_version") != "trace_vb_v3":
    raise SystemExit("Stage 2 requires a question+COMMIT TRACE-VB-v3 checkpoint")
keys = tuple(checkpoint.get("state_dict", {}))
required_fragments = ("trajectory_policy.", "plan_forecaster", "solve_text_decoder.", "sufficiency_head")
...
semantic_anchor_active_during_critic_warmup=false
length_reward=false
semantic_anchor_changes_terminal_reward=false
credit_assignment=role_conditioned_critic_plus_GAE
step_level_RL_signal=per_role_value_targets_and_GAE_advantages_from_terminal_exact_reward
...
ppo_mode=head_only_saved_pre_action_states_plus_one_mean_path_semantic_anchor
...
env \
  ...
  "${TRACE_VB_PYTHON}" run.py \
    ...
    model.model_kwargs.trace_rl_config.semantic_anchor_minimum_weight=0.002 \
    model.model_kwargs.trace_rl_config.semantic_anchor_decay_batches=1536 \
