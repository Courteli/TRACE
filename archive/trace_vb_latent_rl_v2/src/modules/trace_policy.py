...
class GaussianTrajectoryPolicy(nn.Module):
    ...
    def set_stage2_trainability(self):
        """Freeze dynamics/features/COMMIT; refine stochastic role heads.

        The policy trunk and step embeddings are shared by every role.  If
        they moved under a masked PLAN/SOLVE/REFINE objective, the supposedly
        deterministic frozen COMMIT mapping would drift indirectly.  TRACE-VB
        therefore performs literal head-only PPO on the three stochastic
        mean/log-std heads.
        """
