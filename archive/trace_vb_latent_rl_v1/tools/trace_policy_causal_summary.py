...
def load_model(checkpoint_path: Path, device: torch.device):
    ...
    if str(
        ...
        raise ValueError("TRACE-VB causal audit requires path_only")
    ...
    if bool(model.answer_reads_question):
        raise ValueError(
            "TRACE-VB causal evidence forbids direct question attention in "
            "the answer decoder"
        )
    return model
...
def summarize_commit_bottleneck_sanity(
    ...
    *,
    ...
    """Verify the strict COMMIT-only meaning of the requested-prefix curve.

    Columns 0--7 correspond to no COMMIT readout: column zero requests no
    latent and columns 1--7 request private states that the deployed decoder
    is forbidden to read.  Column eight enables COMMIT.  Consequently, the
    first eight columns must agree up to numerical tolerance; this is an
    architectural sanity check and is not a step-level causal attribution.
    """
    ...
    return {
        ...
        "prefix_0_through_7_equal_no_readout": bool(passed),
...
def main():
        ...
        for prefix in range(1, model.n_trace_steps):
            ...
            for metric in ("accuracy", "length", "gold_score"):
                if abs(prefix_result[metric] - no_path[metric]) > 1e-7:
                    raise RuntimeError(
                        "prefix 1--7 must equal no-readout under the strict "
    ...
    if commit_bottleneck_sanity["status"] != "PASS":
        raise RuntimeError(
            "requested pre-COMMIT prefixes violated the path-only readout "
            "contract"
        )
    commit_vs_no_readout = {
    ...
    checks = {
        ...
        "commit_adds_information_over_no_latent_readout": (
            commit_vs_no_readout["accuracy"]["drop_ci95"][0] > 0.0
            and commit_vs_no_readout["gold_score"]["drop_ci95"][0] > 0.0
        ),
    ...
    report = {
        ...
        "answer_context_contract": "path_only_COMMIT",
        "question_attention_access": False,
        "accuracy": accuracy,
        ...
        "commit_vs_no_readout": commit_vs_no_readout,
        ...
        * 7
        ...
        "claim_boundary": (
            "The deployed decoder cannot read the question or private states; "
            "it reads only deterministic COMMIT. Prefix points 1--7 are thus "
            "no-readout controls by construction and make no incremental-step "
        ...
        "gold_score_protocol": (
            "frozen Stage-1 answer adapter under the same path-only COMMIT "
            "mask; the question influences the score only through the "
            "recomputed latent path"
        ),
    }
    ...
    contract = {
        ...
        "diagnostic_only": (
            ...
            "points 1--7 are no-readout controls, not stepwise sufficiency"
        ),
        "controls": [
            "no path",
            "reverse actions",
