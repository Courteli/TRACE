...
validate_checkpoint() {
  ...
  grep -Fq "answer_context_mode: path_only" "${hparams}" || \
    trace_vb_die "${phase} does not enforce path_only"
...
for fragment in ("trajectory_policy.", "plan_forecaster", "sufficiency_head"):
...
if any("trajectory_posterior" in key for key in keys):
    raise SystemExit("TRACE-VB checkpoint unexpectedly contains a posterior")
if expected_stage == 2:
...
answer_decoder_question_attention=false
deployment_policy_CoT_conditioning=false
posthoc_text_CoT_usage=semantic_alignment_audit_only
