...
def build_path_bottleneck_mask(
    ...
    *,
    ...
    The caller supplies the already-resolved latent mask.  TRACE-VB always
    uses ``include_question=False`` and a COMMIT-only latent mask, so answer
    queries read neither the raw question nor the seven private reasoning
    states; all question information must arrive through COMMIT.
    """
    if not (
...
class LitTRACEVB(LitREADCoTStableEfficient):
    ...
    def __init__(self, model_kwargs, training_kwargs, all_config=None):
        ...
        if answer_context_mode != "path_only":
            raise ValueError(
                "TRACE-VB requires answer_context_mode=path_only so the "
                "answer cannot bypass deterministic COMMIT"
            )
        self.answer_context_mode = answer_context_mode
        self.answer_reads_question = False
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        checkpoint["trace_vb_schema_version"] = "trace_vb_v1"
    ...
    def _gold_answer_scores(
        ...
        frozen during Stage 2.  It is evaluated with the same strict path-only,
        COMMIT-only mask as deployment: the question can affect this score only
        through the recomputed latent path.
        """
        target_ids, target_mask = self._prepare_raw_texts(
    ...
    def forward(self, batch):
        ...
        return {
            ...
            "trace_vb_path_only_commit_readout": total_loss.new_ones(()),
            "trace_vb_stochastic_path_count": total_loss.new_ones(()),
            "trace_answer_question_access": total_loss.new_zeros(()),
