...
class RoleLocalActorContractTests(unittest.TestCase):
    def test_stage2_opens_role_features_but_freezes_dynamics_and_commit_head(self):
        ...
        self.assertTrue(any(name.startswith("policy_trunk.") for name in trainable_names))
        self.assertIn("policy_step_embedding.weight", trainable_names)
        self.assertTrue(any(name.startswith("mean_heads.plan.") for name in trainable_names))
        self.assertTrue(any(name.startswith("log_std_heads.solve.") for name in trainable_names))
        self.assertFalse(
        ...
        for frozen_prefix in (
            ...
            self.assertFalse(
                any(name.startswith(frozen_prefix) for name in trainable_names)
            )
