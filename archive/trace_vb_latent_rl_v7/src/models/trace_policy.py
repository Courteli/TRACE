...
def build_path_bottleneck_mask(
    ...
    *,
    ...
    ``include_question=False`` remains available for causal ablations.  The
    caller supplies the already-resolved latent mask; the formal model passes
    a COMMIT-only mask, so answer queries read the question and the final
    committed state but never the seven private reasoning states directly.
    """
    if not (
...
class LitTRACEPolicy(LitREADCoTStableEfficient):
    """Role-semantic latent reasoning with step-level latent-policy RL.

    The eight recurrent states have a fixed computation contract:
    PLAN, five shared-dynamics SOLVE states, CHECK, and deterministic COMMIT.
    Stage 1 distils the original textual CoT into those roles. Stage 2 keeps
    the answer channel frozen and improves only the stochastic latent policy
    with terminal answer reward and per-role CoT semantic reward.
    """
    ...
    def __init__(self, model_kwargs, training_kwargs, all_config=None):
        ...
        if self.n_trace_steps != 8:
            raise ValueError(
                "role-semantic TRACE requires exactly 8 latent states: "
                "PLAN, SOLVE x5, CHECK, COMMIT"
            )
        ...
        if self.do_trace_rl:
            ...
            if invalid_weights:
                raise ValueError(
                    "Role-semantic Stage 2 requires terminal, step-level, and "
                    "Stage-1 trust-region credit; non-positive weights: "
                    f"{invalid_weights}"
                )
    ...
    def _initialize_stage2_modules(self):
        # The Stage-1 answer/path adapter is immutable in Stage 2. Only the
        # role-conditioned latent action density is optimized.
    ...
    def _resolve_latent_read_mask(
        ...
        commit_only = LitTRACEPolicy._commit_only_latent_mask(available)
    ...
    def _gold_answer_scores(
        ...
        The Stage-1 path adapter was explicitly trained on answer-only targets,
        remains frozen during Stage 2, and therefore acts as a stationary
        question-conditioned scorer. Counterfactual comparisons hold the
        question fixed and change only the latent path; this is not a
        path-only architectural bottleneck.
        """
        target_ids, target_mask = self._prepare_raw_texts(
    ...
    def _build_role_semantic_targets(
        ...
        """Build PLAN/SOLVE/CHECK supervision from the one observed CoT.

        The construction is deterministic and label-preserving: PLAN receives a
        global teacher summary, SOLVE receives at most five ordered contiguous
        residual chunks, and CHECK is trained against the remaining error to
        the teacher endpoint. No role annotation or generated rationale is
        required.
        """
        plan_targets = []
        solve_targets = []
        ...
        for explicit_item in explicit_features:
            ...
            direction = states[-1] - states[0]
            plan_targets.append(states.mean(dim=0) + 0.5 * direction)
        ...
        return {
            "plan": torch.stack(plan_targets, dim=0),
    ...
    def _role_semantic_losses(
        ...
        """Compute role losses for [batch, path, eight, hidden] trajectories."""
        ...
        plan_target = role_targets["plan"][:, None, :].expand(
            batch_size,
            path_count,
            -1,
        )
    ...
    def _role_step_rewards(
        ...
        """Return seven train-time CoT semantic rewards and zero for COMMIT."""
        ...
        rewards[:, 0] = masked_cosine_similarity(
            states[:, 0, :],
            role_targets["plan"],
            active,
        )
    ...
    def forward(self, batch):
        """Stage 1: distil one textual CoT into the fixed latent role program."""
        if self.do_trace_rl:
    ...
    def trace_policy_rollout(
        ...
        """Collect grouped latent paths with terminal and per-role rewards."""
        ...
        for start in range(0, len(group_questions), micro_batch):
            ...
            innovation_chunks.append(sampled_path["innovations"].detach())
            ...
            state_chunks.append(
                sampled_path["latent_states"].float().detach()
            )
        ...
        rollout_paths = torch.cat(residual_chunks, dim=0)
        rollout_states = torch.cat(state_chunks, dim=0)
        step_rewards = torch.cat(step_reward_chunks, dim=0)
        ...
        dense_outcome = group_standardize(
        ...
        length_weight = float(
            self.trace_rl_config.get("trajectory_length_weight", 0.05)
        )
        length_penalty = length_weight * F.relu(
            greedy_lengths - target_length
        ) / max(target_length, 1.0)
        terminal_rewards = (
            greedy_accuracy
            + float(self.trace_rl_config.get("dense_outcome_weight", 0.25))
            * dense_outcome
            - length_penalty
        )
        reward_mask = torch.ones_like(step_rewards)
        reward_mask[:, 1:6] = grouped_targets["solve_mask"].to(
            reward_mask.dtype
        )
        reward_mask[:, -1] = 0
        trajectory_advantages = build_discounted_role_advantages(
            terminal_rewards,
            step_rewards,
            group_size=group_size,
            gamma=float(
            ...
            step_reward_weight=float(
                self.trace_rl_config.get("step_reward_weight", 0.30)
            ),
            reward_mask=reward_mask,
        )
        trajectory_returns = build_discounted_role_returns(
            terminal_rewards,
            step_rewards,
            gamma=float(
                self.trace_rl_config.get("step_reward_discount", 0.90)
        ...
        self._last_trace_metrics = {
            ...
            "trace_policy/dense_outcome_abs": dense_outcome.abs().mean(),
            "trace_policy/plan_reward": step_rewards[:, 0].mean(),
            "trace_policy/solve_reward": (
                step_rewards[:, 1:6] * reward_mask[:, 1:6]
            ).sum()
            / reward_mask[:, 1:6].sum().clamp_min(1.0),
            "trace_policy/check_reward": step_rewards[:, 6].mean(),
            "trace_policy/step_reward": (
                step_rewards * reward_mask
            ).sum()
            / reward_mask.sum().clamp_min(1.0),
    ...
    def trace_rl_training_step(
        ...
        """Stage 2: optimize only latent actions with per-role semantic credit."""
        questions = list(batch["question"])
        answers = list(batch["answer"])
        gold_cots = self._decode_single_gold_cots(batch)
        ...
        for _ in range(update_epochs):
            ...
            role_entropies.append(role_entropy)
        ...
        role_entropy = torch.stack(role_entropies).mean()
        ...
        logs = {
            ...
            "train/role_entropy": role_entropy.detach(),
            ...
            "train/semantic_step_reward": (
                rollout["semantic_step_rewards"]
                * rollout["semantic_reward_mask"]
            ).sum()
            / rollout["semantic_reward_mask"].sum().clamp_min(1.0),
