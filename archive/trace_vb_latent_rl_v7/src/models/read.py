...
class LitREADCoT(LitCoTModelBase):
    ...
    def _compact_anchor_step(self, step: str) -> str:
        ...
        if max_chars is not None and len(compact) > int(max_chars):
            limit = int(max_chars)
            if equations:
                # Preserve the final explicit equation first because it is the
                # answer-producing transition; prepend earlier complete
                # equations only while the fixed character budget permits.
                complete = []
                for equation in reversed(equations):
                    candidate = "; ".join([equation] + complete)
                    if complete and len(candidate) > limit:
                        continue
                    if not complete and len(equation) > limit:
                        # Normalized equations are usually shorter than the
                        # limit; retain their suffix rather than an unrelated
                        # early calculation in the exceptional case.
                        equation = equation[-limit:]
                    complete.insert(0, equation)
                compact = "; ".join(complete)
            else:
                words = compact.replace(",", "").replace("$", "").split()
                complete = []
                for word in reversed(words):
                    candidate = " ".join([word] + complete)
                    if complete and len(candidate) > limit:
                        break
                    if not complete and len(word) > limit:
                        word = word[-limit:]
                    complete.insert(0, word)
                compact = " ".join(complete)
        return compact
