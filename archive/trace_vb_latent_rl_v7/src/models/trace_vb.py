    ...
    *,
    ...
    for shard in shards:
        for raw_record in shard:
            if len(raw_record) != 6:
                raise RuntimeError(
                    "TRACE-VB-v7 validation records require index, accuracy, "
...
class LitTRACEVB(LitREADCoTStableEfficient):
    """TRACE-VB-v7 capability-anchored role program.
    ...
    def __init__(self, model_kwargs, training_kwargs, all_config=None):
        ...
        if not self.use_capability_anchor:
            raise ValueError(
                "TRACE-VB-v7 requires the registered capability anchor"
            )
        if bool(
        ...
        if answer_context_mode != "question_and_commit":
            raise ValueError(
                "TRACE-VB-v7 requires answer_context_mode="
        ...
        if not bool(
            ...
            raise ValueError(
                "TRACE-VB-v7 requires the deterministic causal COMMIT "
                "summary transition"
            )
        ...
        if self.do_trace_rl:
            ...
            if not 1 <= update_epochs <= 2:
                raise ValueError(
                    "TRACE-VB-v7 permits one or two evidence-gated PPO "
                    "updates per rollout"
                )
            if bool(self.trace_rl_config.get("use_gae", False)):
                raise ValueError(
                    "TRACE-VB-v7 forbids critic-GAE actor advantages"
                )
            ...
            if invalid_weights:
                raise ValueError(
                    "TRACE-VB-v7 requires positive trust-region and "
    ...
    def configure_optimizers(self):
        ...
        if not self.do_trace_rl:
            ...
            for name, parameter in self.named_parameters():
                ...
                if path_marker in name:
                    ...
                    role_parameters.append(parameter)
            if not path_parameters or not role_parameters:
                raise RuntimeError(
                    "Stage 1 requires both path-LoRA and role parameters"
                )
                ...
                [
                    {
                        ...
                        "lr": float(
                            self.trace_config.get(
                                "stage1_capability_lora_lr", 2.0e-6
                            )
                        ),
                    },
                    {
        ...
        if not actor_parameters:
            raise RuntimeError("TRACE-VB-v7 Stage 2 requires role actor parameters")
    ...
    def load_cot_encoder_state_dict(self, state_dict):
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        checkpoint["trace_vb_schema_version"] = "trace_vb_v7"
    ...
    def on_fit_start(self):
        if not self._capability_anchor_loaded:
            raise RuntimeError(
                "TRACE-VB-v7 requires a fully mapped registered capability "
                "anchor checkpoint"
            )
    ...
    def _stage1_forward_impl(self, batch):
        ...
        if teacher_paths != 1:
            raise ValueError(
                "TRACE-VB-v7 requires exactly one CoT teacher path"
            )
        ...
        return {
            ...
            "trace_stage1_capability_suffix_kl": capability_suffix_kl,
    ...
    def trace_rl_training_step(
        ...
        if not 1 <= update_epochs <= 2:
            raise RuntimeError("formal TRACE-VB-v7 permits at most two updates")
