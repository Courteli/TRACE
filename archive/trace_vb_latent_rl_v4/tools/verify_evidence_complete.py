...
def validate(root: Path, stage1: Path, final: Path) -> dict:
    ...
    if (
        causal.get("answer_context_contract")
        != "question_plus_COMMIT_latent_only"
        or causal.get("question_attention_access") is not True
    ):
        raise SystemExit(
            "causal audit violates the question+COMMIT contract"
        )
    bottleneck = causal.get("commit_bottleneck_sanity", {})
    if (
        bottleneck.get("status") != "PASS"
        or bottleneck.get("prefix_0_through_7_equal_question_only") is not True
    ...
    if (
        figure_contract.get("answer_context_contract")
        != "question_plus_COMMIT_latent_only"
        or int(figure_contract.get("question_attention_access", -1)) != 1
