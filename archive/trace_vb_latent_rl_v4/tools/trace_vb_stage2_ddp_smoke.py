...
def main() -> None:
    ...
    optimizer = unwrap_optimizer(model.configure_optimizers())
    if len(optimizer.param_groups) != 2:
        raise RuntimeError("Stage-2 DDP smoke requires actor/critic optimizer groups")
    if float(optimizer.param_groups[0]["lr"]) != float(rl.actor_lr):
        raise RuntimeError("Stage-2 DDP actor LR mismatch")
    if float(optimizer.param_groups[1]["lr"]) != float(rl.critic_lr):
    ...
    if warmup_actor_change != 0.0 or not warmup_critic_change > 0.0:
        raise RuntimeError("warmup step changed actor or failed to change critic")
