...
def build_path_bottleneck_mask(
    ...
    *,
    ...
    The caller supplies the already-resolved latent mask.  The formal VB-v4
    ...
    *,
    ...
    for shard in shards:
        for raw_record in shard:
            if len(raw_record) != 6:
                raise RuntimeError(
                    "TRACE-VB-v4 validation records require index, accuracy, "
...
class LitTRACEVB(LitREADCoTStableEfficient):
    ...
    def __init__(self, model_kwargs, training_kwargs, all_config=None):
        ...
        if bool(
            ...
            raise ValueError(
                "TRACE-VB-v4 forbids saved-tensor CPU activation offload: "
        ...
        if answer_context_mode != "question_and_commit":
            raise ValueError(
                "TRACE-VB-v4 requires answer_context_mode="
        ...
        # V4 keeps exactly one training-only CoT teacher path and one
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        checkpoint["trace_vb_schema_version"] = "trace_vb_v4"
    ...
    def _stage1_forward_impl(self, batch):
        ...
        if teacher_paths != 1:
            raise ValueError(
                "TRACE-VB-v4 requires exactly one CoT teacher path"
            )
    ...
    def on_validation_epoch_end(self):
        ...
        if getattr(self.trainer, "is_global_zero", True):
            ...
            directory.mkdir(parents=True, exist_ok=True)
            report = {
                "schema_version": "trace_vb_v4_validation_behavior_v1",
