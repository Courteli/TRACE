...
if checkpoint.get("trace_policy_version") != "TRACE-Policy-v3":
    raise SystemExit("evidence requires TRACE-Policy-v3 checkpoints")
...
OUT=${OUT:-${ROOT}/run_outputs/evidence/${RUN_TAG}}
...
run_test() {
  ...
  env \
    ...
    "${PYTHON}" run.py \
      ...
      data_module.dataset_dir="${ROOT}/data/raw/GSM8k-Aug-NL" \
...
gsm8k_source=${ROOT}/data/raw/GSM8k-Aug-NL
...
"${PYTHON}" tools/trace_prepare_pca_fit_set.py \
  --source-file "${ROOT}/data/raw/GSM8k-Aug-NL/gsm8k_train_processed.jsonl" \
...
run_evidence_suite() {
  ...
  run_test "${prefix}_gsmhard" "${checkpoint}" \
    ...
    data_module.dataset_dir="${ROOT}/data/raw/GSM8K-Hard" \
  ...
  run_test "${prefix}_svamp" "${checkpoint}" \
    ...
    data_module.dataset_dir="${ROOT}/data/raw/SVAMP" \
  ...
  run_test "${prefix}_multiarith" "${checkpoint}" \
    ...
    data_module.dataset_dir="${ROOT}/data/raw/MultiArith" \
