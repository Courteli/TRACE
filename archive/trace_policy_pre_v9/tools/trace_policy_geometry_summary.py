...
Core conclusion:
    ...
    outcome-valid local modes reject nearby invalid paths, and the complete
    eight-step path contributes information beyond the question context.
Evidence chain:
...
def plot_corridor_assignments(record: dict, output_base: Path):
    ...
    fig.suptitle(
        f"One-CoT corridor alignments | q{record['idx']}",
        x=0.02,
        ha="left",
        fontsize=8,
        fontweight="bold",
...
def plot_formation_evidence(
    ...
    axes[0].set_title(
        "Actions induce ordered schedules",
        loc="left",
        fontweight="bold",
    )
    ...
    axes[1].set_title(
        "Policy choices survive in paths",
        loc="left",
        fontweight="bold",
    )
    ...
    axes[2].set_title(
        "Coupling exceeds shuffled pairing",
        loc="left",
        fontweight="bold",
    )
...
def main():
    ...
    for record, actions, centers, distance, outcome in zip(
        records,
        action_tensors,
        progress_centers,
        distance_matrices,
    ...
    plot_formation_evidence(
        ...
        args.output_dir / "formation_action_coupling",
    )
    for index in selected:
        ...
        plot_corridor_assignments(
            record,
            args.output_dir / f"corridor_assignment_q{record['idx']}",
        )
    ...
    markdown = [
        ...
        f"- Action-conditioned progress diversity: "
        f"**{metrics['progress_schedule_diversity']['mean']:.4f}** "
        f"(95% CI {metrics['progress_schedule_diversity']['ci95'][0]:.4f}, "
        f"{metrics['progress_schedule_diversity']['ci95'][1]:.4f}).",
        f"- Action--path distance correlation: "
        f"**{metrics['action_path_distance_correlation']['mean']:.3f}**; "
        f"within-question pairing null excess "
        f"**{metrics['action_path_permutation_null']['excess']:+.3f}**, "
        f"`p={metrics['action_path_permutation_null']['p_value']:.4g}`.",
        "",
