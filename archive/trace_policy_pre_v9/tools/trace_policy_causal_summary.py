...
Review risks:
    The decoder is architecturally path-only, but a bottleneck alone does not
    prove that every transition is behaviorally used. No-path, prefix, order,
    and all-eight transition controls test those distinct necessity claims.
"""
...
def load_model(checkpoint_path: Path, device: torch.device):
    ...
    if checkpoint.get("trace_policy_version") != "TRACE-Policy-v3":
        raise ValueError("causal audit requires TRACE-Policy-v3")
    ...
    if model.answer_reads_question:
        raise ValueError("causal audit refuses a decoder with question bypass")
    return model
...
def main():
    ...
    checks = {
        "complete_path_is_necessary_under_architectural_bottleneck": (
    ...
    report = {
        ...
        "claim_boundary": (
            "Question K/V is architecturally masked from the answer decoder. "
            "Passing no-path and prefix tests supports path necessity and "
            "sufficiency; passing all eight replacement gates supports use of "
            "every transition. A failed gate narrows the claim and cannot be "
            "replaced by a selected 3D view. Transition-wide inference uses "
            "family-wise 95% Bonferroni bootstrap intervals."
        ),
        "gold_score_protocol": (
            "frozen Stage-1 path-only direct-answer scorer"
        ),
    }
