...
class LitREADCoT(LitCoTModelBase):
    ...
    def _normalize_compact_equation(self, equation: str) -> str:
        ...
        # Currency marks and thousands separators carry no arithmetic meaning
        # but consume several Qwen tokens in the fixed deployment budget.
        equation = equation.replace(",", "").replace("$", "")
        equation = re.sub(r"(?<=\d)\s*[xX]\s*(?=\d)", "*", equation)
        equation = re.sub(r"\s*([+\-*/=])\s*", r"\1", equation)
        return equation.strip(" .,;")

    def _complete_compact_equations(self, step: str) -> List[str]:
        """Return complete, bounded equations without character slicing.

        TRACE-VB-v8 treats a compact equation as an atomic semantic unit.  An
        overlong equation is omitted in full rather than retaining a suffix
        whose operands may no longer be valid.  Duplicate equations within a
        CoT step are removed while preserving their causal order.
        """
        step = re.sub(r"\s+", " ", str(step).strip())
        if not step:
            return []
        max_chars = self.readcot_config.get(
            "compact_anchor_max_chars", None
        )
        if max_chars is not None and int(max_chars) <= 0:
            raise ValueError("compact_anchor_max_chars must be positive")
        limit = None if max_chars is None else int(max_chars)
        equations = []
        seen = set()
        for match in _COMPACT_EQUATION_PATTERN.finditer(step):
            equation = self._normalize_compact_equation(match.group(0))
            if not equation or (limit is not None and len(equation) > limit):
                continue
            if equation in seen:
                continue
            seen.add(equation)
    ...
    def _compact_anchor_step(self, step: str) -> str:
        # No prose or loose token fallback: if a complete equation is absent,
        # the v8 deployment target goes directly to the protected answer.
        return "; ".join(self._complete_compact_equations(step))
