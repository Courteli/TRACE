...
class LitCoLaR(LitCoTModelBase):
    ...
    def forward(self, batch):
        ...
        extra_sft_log_dict = self.extra_sft_losses(
            batch=batch,
            r=r,
            question_inputs_embeds=question_inputs_embeds,
            question_attention_mask=question_attention_mask,
            steps_inputs_embeds=steps_inputs_embeds,
            steps_attention_mask=steps_attention_mask,
            answer_inputs_embeds=answer_inputs_embeds,
            answer_attention_mask=answer_attention_mask,
            steps_outputs=steps_outputs,
            distributions=distributions,
            pred_embeds=pred_embeds,
            gold_embeds=gold_embeds,
            gold_embeds_norm=gold_embeds.detach() / self.embeds_std,
        )
        extra_sft_loss = extra_sft_log_dict.pop("extra_sft_loss", 0.0)
        ...
        if isinstance(extra_sft_loss, torch.Tensor):
            total_loss += extra_sft_loss
        elif extra_sft_loss != 0:
            total_loss += torch.as_tensor(extra_sft_loss, device=ce_loss.device, dtype=ce_loss.dtype)
        ...
        log_dict.update(extra_sft_log_dict)
        return log_dict
    ...
    def get_logprobs(self, e: grpo.Experience):
        ...
        latent_logprobs = torch.nan_to_num(latent_logprobs, nan=-30.0, neginf=-30.0, posinf=30.0)
        latent_logprobs = latent_logprobs.clamp(min=-30.0, max=30.0)
        ...
        flat_answer_logits = answer_logits.reshape(-1, answer_logits.shape[-1])
        flat_answer_ids = e.answer_input_ids.reshape(-1)
        selected_answer_logprobs = []
        # Avoid materializing the full [group, answer_len, vocab] log-prob tensor in RL.
        for start in range(0, flat_answer_logits.shape[0], 16):
            end = min(start + 16, flat_answer_logits.shape[0])
            chunk_logprobs = F.log_softmax(flat_answer_logits[start:end], dim=-1)
            selected_answer_logprobs.append(
                chunk_logprobs.gather(dim=-1, index=flat_answer_ids[start:end].unsqueeze(-1)).squeeze(-1)
            )
        answer_logprobs = torch.cat(selected_answer_logprobs, dim=0).reshape_as(e.answer_input_ids)
        answer_logprobs = torch.nan_to_num(answer_logprobs, nan=-30.0, neginf=-30.0, posinf=30.0)
        answer_logprobs = answer_logprobs.clamp(min=-30.0, max=30.0)
