...
---------------
Core conclusion:
    Stage-2 is judged by paired task behavior and outcome calibration while
    preserving PLAN/SOLVE/REFINE semantics and a strictly deterministic,
    answer-visible COMMIT transition.
Evidence logic:
    Every statistic is paired by the same 200 GSM8K test questions and eight
    IID role-conditioned paths per question. Paired behavior, critic outcome
    calibration, and causal path dependence are primary. Recorded role cosine,
    with SOLVE aggregated only over observed teacher chunks, is a retention
    diagnostic. Outcome/corridor display geometry remains diagnostic under one
    shared train-fit PCA contract.
Review risks:
...
def load_records(path: Path):
    ...
    required = (
        ...
        "map_correct",
        "map_output_length",
        "answer_question_attention_access",
        ...
        "rollout_value_predictions",
        "map_value_predictions",
        "value_head_type",
        "value_action_mask",
    ...
    for row, record in enumerate(records):
        ...
        rollout_values = torch.as_tensor(
            record["rollout_value_predictions"]
        ).float()
        map_values = torch.as_tensor(record["map_value_predictions"]).float()
        value_mask = torch.as_tensor(record["value_action_mask"]).bool()
        if rollout_values.shape != (8, 8) or map_values.shape != (8,):
            raise ValueError(f"record {row} has invalid value predictions")
        if (
            value_mask.shape != (8,)
            or not bool(value_mask[:7].all())
            or bool(value_mask[7])
        ):
            raise ValueError(f"record {row} has invalid value action mask")
        if str(record["value_head_type"]) not in {
            "sufficiency",
            "outcome_critic",
        }:
            raise ValueError(f"record {row} has invalid value head type")
        if (
            not bool(torch.isfinite(rollout_values).all())
            or not bool(torch.isfinite(map_values).all())
            or not bool(((rollout_values >= 0) & (rollout_values <= 1)).all())
            or not bool(((map_values >= 0) & (map_values <= 1)).all())
        ):
            raise ValueError(f"record {row} has invalid probability values")
        ...
        if teacher_summary.shape != (hidden_size,):
            raise ValueError(f"record {row} has an invalid REFINE summary")
        ...
        if int(record["answer_question_attention_access"]) != 1:
            raise ValueError(
                f"record {row} lost the raw-question answer bridge"
            )
...
def value_arrays(records):
    """Return paired question-level and path-level terminal value evidence."""
    head_types = {str(record["value_head_type"]) for record in records}
    if len(head_types) != 1:
        raise ValueError(
            "all records in one stage must use the same value head type"
        )
    predictions = np.stack(
        [
            torch.as_tensor(record["rollout_value_predictions"])
            .float()
            .numpy()
            for record in records
        ]
    ).astype(np.float64)
    map_predictions = np.stack(
        [
            torch.as_tensor(record["map_value_predictions"])
            .float()
            .numpy()
            for record in records
        ]
    ...
    map_outcomes = np.asarray(
        [record["map_correct"] for record in records],
        dtype=np.float64,
    )
    masks = np.stack(
        [
            torch.as_tensor(record["value_action_mask"])
            .bool()
            .numpy()
            for record in records
        ]
    )
    if not np.all(masks == np.asarray([True] * 7 + [False])):
        raise ValueError("value masks diverge from seven stochastic actions")
    expanded_outcomes = outcomes[:, :, None]
    active_error = (
        predictions[:, :, :7] - expanded_outcomes
    ) ** 2
    final_values = predictions[:, :, 6]
    return {
        "head_type": head_types.pop(),
        "question_all_role_brier": active_error.mean(axis=(1, 2)),
        "question_final_action_brier": (
            (final_values - outcomes) ** 2
        ).mean(axis=1),
        "question_map_final_action_brier": (
            map_predictions[:, 6] - map_outcomes
        ) ** 2,
        "final_action_values": final_values.reshape(-1),
        "final_action_outcomes": outcomes.reshape(-1),
        "all_role_values": predictions[:, :, :7],
        "map_values": map_predictions,
    }


def calibration_summary(values, outcomes, bins: int = 10):
    values = np.asarray(values, dtype=np.float64).reshape(-1)
    outcomes = np.asarray(outcomes, dtype=np.float64).reshape(-1)
    if values.shape != outcomes.shape or values.size == 0:
        raise ValueError("value calibration inputs must be non-empty and paired")
    if not (
        np.isfinite(values).all()
        and np.isfinite(outcomes).all()
        and np.all((values >= 0.0) & (values <= 1.0))
        and np.all((outcomes == 0.0) | (outcomes == 1.0))
    ):
        raise ValueError("value calibration received invalid probabilities/outcomes")
    ...
    for index in range(int(bins)):
        ...
        if not np.any(selected):
            ...
            continue
        prediction = float(values[selected].mean())
        accuracy = float(outcomes[selected].mean())
        count = int(selected.sum())
        ece += count / values.size * abs(prediction - accuracy)
        rows.append(
            {
                "lower": float(edges[index]),
                "upper": float(edges[index + 1]),
    ...
    return {
        ...
        "empirical_accuracy": float(outcomes.mean()),
        "correct_mean_value": (
            float(values[correct].mean()) if np.any(correct) else None
        ),
        "wrong_mean_value": (
            float(values[wrong].mean()) if np.any(wrong) else None
        ),
        "correct_wrong_gap": (
            float(values[correct].mean() - values[wrong].mean())
            if np.any(correct) and np.any(wrong)
            else None
        ),
        "bins": rows,
    }
...
def plot_value_calibration(
    stage1_summary: Dict[str, object],
    final_summary: Dict[str, object],
    output_base: Path,
):
    """Plot outcome calibration of Stage-1 S and calibrated Stage-2 V."""
    fig, axes = plt.subplots(1, 2, figsize=(5.25, 2.35))
    axes[0].plot([0, 1], [0, 1], color=INK, linestyle="--", linewidth=0.9)
    for summary, color, marker, label in (
        (stage1_summary, GREEN, "o", "Stage 1 sufficiency S"),
        (final_summary, PINK_DARK, "s", "Stage 2 outcome V"),
    ):
        rows = [row for row in summary["bins"] if row["count"] > 0]
        axes[0].plot(
            [row["mean_prediction"] for row in rows],
            [row["empirical_accuracy"] for row in rows],
            color=color,
            marker=marker,
            markersize=4,
            linewidth=1.4,
    ...
    axes[0].set_xlim(0, 1)
    axes[0].set_ylim(0, 1)
    axes[0].set_xlabel("Predicted terminal success")
    axes[0].set_ylabel("Empirical correctness")
    axes[0].set_title("a  Terminal-outcome reliability", loc="left", fontweight="bold")
    axes[0].grid(color=GRID, linewidth=0.5)
    axes[0].legend(loc="best", fontsize=5.6)

    labels = ("Brier", "ECE")
    left = np.asarray([stage1_summary["brier"], stage1_summary["ece_10bin"]])
    right = np.asarray([final_summary["brier"], final_summary["ece_10bin"]])
    x = np.arange(2)
    width = 0.34
    axes[1].bar(x - width / 2, left, width, color=GREEN, label="Stage 1 S")
    axes[1].bar(x + width / 2, right, width, color=PINK, edgecolor=PINK_DARK, label="Stage 2 V")
    axes[1].set_xticks(x, labels)
    axes[1].set_ylabel("Error (lower is better)")
    axes[1].set_title("b  Calibration error", loc="left", fontweight="bold")
    axes[1].grid(axis="y", color=GRID, linewidth=0.5)
    axes[1].legend(loc="best", fontsize=5.8)
    fig.tight_layout(pad=0.65, w_pad=1.1)
    save_figure(fig, output_base)
...
def main():
    ...
    stage1_value = value_arrays(stage1_records)
    final_value = value_arrays(final_records)
    if stage1_value["head_type"] != "sufficiency":
        raise ValueError("Stage-1 records do not use the sufficiency head")
    if final_value["head_type"] != "outcome_critic":
        raise ValueError("Final records do not use the calibrated outcome critic")
    rng = np.random.default_rng(20260719)

    semantic_names = (
        "map_plan_similarity",
        "map_solve_similarity",
        "map_refine_similarity",
        "rollout_plan_similarity",
        "rollout_solve_similarity",
        "rollout_refine_similarity",
    )
    role_semantics = {
        key: paired_ci(
            stage1_role[key],
    ...
    calibration_metric_names = (
        "question_all_role_brier",
        "question_final_action_brier",
        "question_map_final_action_brier",
    )
    terminal_value_calibration = {
        "stage1_head_type": stage1_value["head_type"],
        "final_head_type": final_value["head_type"],
        "stage1": calibration_summary(
            stage1_value["final_action_values"],
            stage1_value["final_action_outcomes"],
        ),
        "final": calibration_summary(
            final_value["final_action_values"],
            final_value["final_action_outcomes"],
        ),
        "paired_question_metrics": {
            key: paired_ci(
                stage1_value[key],
                final_value[key],
                rng=rng,
            ...
            for key in calibration_metric_names
        },
        "target": "terminal_exact_correctness",
        "evaluated_state": "pre_action_REFINE_state_at_role_index_6",
    }
    ...
    report = {
        "status": "PASS",
        "core_conclusion": (
            "Primary evidence compares paired task behavior and terminal "
            "outcome calibration while auditing deterministic COMMIT. "
            "PLAN/SOLVE/REFINE cosine is a retention diagnostic."
        ),
        "evidence_priority": {
            "primary": "behavior_and_terminal_outcome_calibration",
            "supporting": ["commit_determinism", "causal_path_dependence"],
            "retention_diagnostic": ["role_semantics", "role_exploration"],
            "diagnostic_only": "display_geometry",
        },
        ...
        "terminal_outcome_value_calibration": terminal_value_calibration,
    ...
    append_metric_table(
        "Retention diagnostic: PLAN/SOLVE/REFINE teacher-semantic alignment",
        role_semantics,
        semantic_names,
        (
    ...
    append_metric_table(
        ...
        (
            ...
            "only for stochastic PLAN/SOLVE/REFINE slots; COMMIT is excluded."
        ),
    )
    lines.extend(
        [
    ...
    append_metric_table(
        "Primary: paired behavioral outcomes",
        behavior,
        (
            "map_accuracy",
            "map_total_length",
    ...
    append_metric_table(
        "Primary: terminal-outcome value calibration",
        terminal_value_calibration["paired_question_metrics"],
        calibration_metric_names,
        (
            "Stage 1 reports semantic sufficiency S; Final reports the "
            "outcome-calibrated critic V. Negative Brier delta is better."
        ),
    )
    append_metric_table(
    ...
    plot_value_calibration(
        terminal_value_calibration["stage1"],
        terminal_value_calibration["final"],
        args.output_dir / "stage_value_calibration",
    )
    ...
    np.savez_compressed(
        ...
        stage1_rollout_value_predictions=stage1_value["all_role_values"],
        final_rollout_value_predictions=final_value["all_role_values"],
        stage1_final_action_values=stage1_value["final_action_values"],
        final_final_action_values=final_value["final_action_values"],
        stage1_terminal_outcomes=stage1_value[
            "final_action_outcomes"
        ],
        final_terminal_outcomes=final_value["final_action_outcomes"],
    ...
    contract = {
        "core_conclusion": (
            "Stage-2 is evaluated by paired behavior and calibrated terminal "
            "outcome prediction while preserving PLAN/SOLVE/REFINE semantics "
            "and a deterministic, answer-visible COMMIT transition."
        ),
        "figure_archetype": "quantitative grid",
        "evidence_hierarchy": {
            "hero": "stage_accuracy_length",
            "primary": [
                "stage_reliability",
                "stage_value_calibration",
            ],
            "supporting": ["causal_summary", "commit_determinism"],
            "retention_diagnostic": ["stage_role_evidence"],
            "diagnostic_only": [
        ...
        "retention_panel_map": {
            "a": "MAP PLAN/SOLVE/REFINE semantic retention",
            "b": "eight-path PLAN/SOLVE/REFINE semantic retention",
            "c": (
        ...
        "value_calibration": terminal_value_calibration,
    ...
    qa = figure_qa(
        args.output_dir,
        expected_figures=5 + 2 * len(selected),
    )
