...
def main() -> None:
    ...
    if getattr(model, "trajectory_posterior", None) is None:
        raise RuntimeError("TRACE-VB DDP smoke requires one CoT teacher posterior")
    ...
    for step in range(args.optimizer_steps):
        ...
        if int(output.get("trace_answer_question_access", loss.new_zeros(())).item()) != 1:
            raise RuntimeError(f"rank {rank} answer readout lost question access")
        if int(output.get("trace_vb_private_latent_answer_access", loss.new_ones(())).item()) != 0:
            raise RuntimeError(f"rank {rank} answer readout bypassed COMMIT")
        loss.backward()
