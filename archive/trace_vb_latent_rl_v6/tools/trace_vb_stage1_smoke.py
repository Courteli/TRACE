...
def main() -> None:
    ...
    config = build_config()
    if str(config.model.model_kwargs.trace_policy_config.answer_context_mode) != "question_and_commit":
        raise RuntimeError("Stage-1 smoke did not load question_and_commit")
    ...
    if int(config.model.model_kwargs.trace_policy_config.stage1_posterior_samples) != 1:
        raise RuntimeError("Stage-1 smoke requires exactly one CoT teacher path")
    policy_config = config.model.model_kwargs.trace_policy_config
    if not bool(config.model.model_kwargs.trace_policy_config.stage1_answer_activation_checkpoint):
        raise RuntimeError("Stage-1 smoke requires answer activation checkpointing")
    if (
    ...
    if (
        float(policy_config.stage1_solve_text_weight) <= 0.0
        or float(policy_config.stage1_solve_weight)
        >= float(policy_config.stage1_solve_text_weight)
    ):
        raise RuntimeError("Stage-1 text decoding is not the primary SOLVE loss")
    ...
    if getattr(model, "trajectory_posterior", None) is None:
        raise RuntimeError("TRACE-VB must instantiate one CoT teacher posterior")
    if not hasattr(model, "posterior_context_norm"):
        raise RuntimeError("TRACE-VB is missing posterior context normalization")
    ...
    if not bool(model.answer_reads_question):
        raise RuntimeError("question+COMMIT model did not expose the raw question")
    ...
    for case in stress_cases:
        ...
        text_keys = (
            "trace_vb_solve_text_loss",
            "trace_vb_solve_text_active_chunks",
            "trace_vb_solve_text_tokens_per_chunk",
            "trace_vb_solve_text_truncated_fraction",
            "trace_stage1_map_answer_suffix_loss",
            "trace_vb_commit_loss",
            "lambda_solve_text_eff",
            "lambda_map_answer_suffix_eff",
            "lambda_commit_eff",
        )
        missing_text = [key for key in text_keys if key not in output]
        if missing_text:
            raise RuntimeError(
                "Stage-1 output is missing text supervision: "
                + ", ".join(missing_text)
            )
        if not all(
            ...
            raise RuntimeError("Stage-1 text-supervision outputs are non-finite")
        if float(output["trace_vb_solve_text_truncated_fraction"]) != 0.0:
            raise RuntimeError("Stage-1 smoke detected CoT truncation")
        expected_chunks = 5.0 * float(len(batch["question"]))
        if float(output["trace_vb_solve_text_active_chunks"]) != expected_chunks:
            raise RuntimeError(
                "Stage-1 smoke did not supervise every SOLVE role: "
                f"expected {expected_chunks}, got "
                f"{float(output['trace_vb_solve_text_active_chunks'])}"
            )
        if not output["trace_vb_solve_text_loss"].requires_grad:
            raise RuntimeError("Stage-1 text decoder loss is detached")
        for key in (
            "trace_stage1_map_answer_suffix_loss",
            "trace_vb_commit_loss",
        ):
            if not output[key].requires_grad:
                raise RuntimeError(f"Stage-1 objective is detached: {key}")
        if float(output["lambda_map_answer_suffix_eff"]) <= 0.0:
            raise RuntimeError("protected map answer suffix has zero weight")
        if float(output["lambda_commit_eff"]) <= 0.0:
            raise RuntimeError("COMMIT semantic alignment has zero weight")
        efficacy_keys = (
        ...
        reconstructed_loss = (
            ...
            + output["lambda_posterior_kl_eff"]
            * output["trace_stage1_posterior_kl"]
            ...
            * output["trace_vb_plan_forecast_loss"]
            ...
            + output["lambda_commit_eff"] * output["trace_vb_commit_loss"]
            ...
            * output["trace_vb_variance_floor_loss"]
            + output["lambda_solve_text_eff"]
            * output["trace_vb_solve_text_loss"]
        ...
        gradients = {
            ...
            "solve_text_decoder": module_grad_norm(model.solve_text_decoder),
        ...
        for key in (
            "trace_vb_deployment_compact_reasoning",
            "trace_vb_question_only_deployment_path",
            "trace_vb_training_only_cot_teacher_path",
            "trace_vb_question_commit_readout",
            "trace_vb_commit_residual_bridge",
            "trace_vb_teacher_posterior_count",
            "trace_vb_answer_activation_checkpoint",
        ):
            if key not in output or int(output[key].item()) != 1:
                raise RuntimeError(f"Stage-1 runtime contract failed: {key}")
        if int(output.get("trace_answer_question_access", loss.new_zeros(())).item()) != 1:
            raise RuntimeError("Stage-1 answer readout did not access the raw question")
        if int(output.get("trace_vb_private_latent_answer_access", loss.new_ones(())).item()) != 0:
            raise RuntimeError("Stage-1 answer readout bypassed COMMIT")
        ...
        reports.append(
            {
                **case,
                ...
                "solve_text_loss": float(
                    output["trace_vb_solve_text_loss"].detach().float().cpu()
                ),
                "commit_loss": float(
                    output["trace_vb_commit_loss"].detach().float().cpu()
                ),
                "map_answer_suffix_loss": float(
                    output["trace_stage1_map_answer_suffix_loss"]
                    .detach().float().cpu()
                ),
                "solve_text_truncated_fraction": float(
                    output["trace_vb_solve_text_truncated_fraction"]
                    .detach().float().cpu()
                ),
    ...
    report = {
        ...
        "answer_context": "question_and_commit",
        "stochastic_paths": 1,
        "posterior_paths": 0,
