...
validate_checkpoint() {
  ...
  if ! grep -Fq "workspace_path: ${CODE_ROOT}" "${hparams}"; then
    echo "${phase} checkpoint was not produced by the new isolated code root" >&2
    exit 2
  fi
...
if not any(".trace_cot_encoder." in key for key in keys):
    raise SystemExit("checkpoint is missing the frozen single-CoT encoder")
...
run_test() {
  ...
  env \
    ...
    "${PYTHON}" run.py \
      ...
      data_module.dataset_dir="${DATA_ROOT}/data/raw/GSM8k-Aug-NL" \
...
total_L_definition=8_latent_states_plus_all_generated_compact_equation_and_answer_tokens
gsm8k_source=${DATA_ROOT}/data/raw/GSM8k-Aug-NL
...
posthoc_text_CoT_usage=role_alignment_scoring_only
...
"${PYTHON}" tools/trace_prepare_pca_fit_set.py \
  --source-file "${DATA_ROOT}/data/raw/GSM8k-Aug-NL/gsm8k_train_processed.jsonl" \
...
run_evidence_suite() {
  ...
  run_test "${prefix}_gsmhard" "${checkpoint}" \
    ...
    data_module.dataset_dir="${DATA_ROOT}/data/raw/GSM8K-Hard" \
  ...
  run_test "${prefix}_svamp" "${checkpoint}" \
    ...
    data_module.dataset_dir="${DATA_ROOT}/data/raw/SVAMP" \
  ...
  run_test "${prefix}_multiarith" "${checkpoint}" \
    ...
    data_module.dataset_dir="${DATA_ROOT}/data/raw/MultiArith" \
