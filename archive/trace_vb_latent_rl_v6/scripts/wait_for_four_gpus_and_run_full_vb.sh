...
PIPELINE_TAG=${PIPELINE_TAG:-$(date +%Y%m%d-%H%M%S)_trace_vb_v5_full_seed${TRAIN_SEED}}
...
exec 9>"${supervisor_dir}/.trace_vb_v5_full_pipeline.lock"
if ! flock -n 9; then
  trace_vb_die "a TRACE-VB-v5 full-pipeline waiter or run is already active"
fi
...
while true; do
  ...
  if (( ${#eligible_gpus[@]} >= 4 )); then
    ...
    echo "$(date --iso-8601=seconds) launching TRACE-VB-v5 full pipeline on ${selected}"
