    ...
    *,
    ...
    for shard in shards:
        for raw_record in shard:
            if len(raw_record) != 6:
                raise RuntimeError(
                    "TRACE-VB-v5 validation records require index, accuracy, "
...
class LitTRACEVB(LitREADCoTStableEfficient):
    ...
    def __init__(self, model_kwargs, training_kwargs, all_config=None):
        ...
        if bool(
            ...
            raise ValueError(
                "TRACE-VB-v5 forbids saved-tensor CPU activation offload: "
        ...
        if answer_context_mode != "question_and_commit":
            raise ValueError(
                "TRACE-VB-v5 requires answer_context_mode="
        ...
        if not bool(
            self.trace_config.get("commit_residual_bridge", True)
        ):
            raise ValueError(
                "TRACE-VB-v5 requires the deterministic REFINE-to-COMMIT "
                "residual bridge"
            )
        ...
        if self.do_trace_rl:
            ...
            if not 1 <= update_epochs <= 2:
                raise ValueError(
                    "TRACE-VB-v6 permits one or two evidence-gated PPO "
                    "updates per rollout"
                )
            if bool(self.trace_rl_config.get("use_gae", False)):
                raise ValueError(
                    "TRACE-VB-v6 forbids critic-GAE actor advantages"
                )
            positive_stage2_weights = ("stage1_policy_kl_weight",)
            invalid_weights = [
                key
            ...
            if invalid_weights:
                raise ValueError(
                    "TRACE-VB-v6 requires a positive Stage-1 trust-region; "
    ...
    def _initialize_stage2_modules(self):
        ...
        # channel are immutable. Only the stochastic Gaussian output heads are
        # optimized; the former value critic remains frozen for checkpoint
        # compatibility and cannot manufacture actor preferences.
    ...
    def configure_optimizers(self):
        """Optimize only stochastic actor heads during TRACE-VB-v6 Stage 2."""
        ...
        if not actor_parameters:
            raise RuntimeError("TRACE-VB-v6 Stage 2 requires actor heads")
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        checkpoint["trace_vb_schema_version"] = (
            "trace_vb_v6" if checkpoint_stage == 2 else "trace_vb_v5"
        )
        checkpoint["trace_vb_value_bridge_initialized"] = False
        checkpoint["trace_vb_stage2_objective"] = (
            "evidence_gated_group_latent_rl"
            if checkpoint_stage == 2
            else "role_semantic_cot_sft"
        )
        if checkpoint_stage == 2:
    ...
    def _trajectory_latents(
        ...
        *,
        ...
        for step_index in range(self.n_trace_steps):
            ...
            # during the at-most-two cached-state head-only updates.
            ...
            if step_index == self.n_trace_steps - 1:
                # COMMIT is the only private-role state exposed to the answer
                # decoder. Preserve the already summary-aligned REFINE state
                # through an explicit residual instead of asking a fresh
                # transformer token to rediscover the complete solution.
                current_state = self.state_norm(
    ...
    def _stage1_forward_impl(self, batch):
        ...
        if teacher_paths != 1:
            raise ValueError(
                "TRACE-VB-v5 requires exactly one CoT teacher path"
            )
        ...
        map_compact_loss, map_answer_suffix_loss = (
        ...
        return {
            ...
            "trace_vb_commit_residual_bridge": total_loss.new_ones(()),
    ...
    def trace_policy_rollout(
        ...
        """Collect group-8 paths and construct fail-closed local evidence."""
        forbidden_shaping = (
    ...
    def trace_rl_training_step(
        ...
        actor_active = batches_seen_before >= calibration_batches
        if (
        ...
        if not 1 <= update_epochs <= 2:
            raise RuntimeError("formal TRACE-VB-v6 permits at most two updates")
    ...
    def on_validation_epoch_end(self):
        ...
        if getattr(self.trainer, "is_global_zero", True):
            ...
            directory.mkdir(parents=True, exist_ok=True)
            report = {
                "schema_version": "trace_vb_v5_validation_behavior_v1",
