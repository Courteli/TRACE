...
contains the training-only PLAN forecast head, role-semantic helpers, and the
small policy objectives used by TRACE-VB.  A caller supplies the
stochastic-action mask, so deterministic COMMIT states can be retained in
rollout tensors while being excluded from every policy objective.
"""
...
class EvidenceGatedGroupOutput(NamedTuple):
    """Question-local rewards, advantages, and auditable group evidence."""

    policy_rewards: torch.Tensor
    path_advantages: torch.Tensor
    path_informative_mask: torch.Tensor
    failure_entropy_mask: torch.Tensor
    all_correct_group_mask: torch.Tensor
    mixed_group_mask: torch.Tensor
    all_wrong_group_mask: torch.Tensor
    likelihood_group_mask: torch.Tensor
    gold_score_spread: torch.Tensor
    proxy_pair_credit: torch.Tensor
    proxy_pair_count: torch.Tensor
...
def evidence_gated_group_advantages(
    exact_correctness: torch.Tensor,
    gold_answer_scores: torch.Tensor,
    *,
    group_size: int,
...
) -> EvidenceGatedGroupOutput:
    """Build fail-closed, question-local latent-policy evidence.

    Mixed correct/wrong groups use exact correctness.  Homogeneous failures
    may use a rank of the frozen gold-answer log-likelihood, but only after a
    separate calibration gate has enabled that proxy and the within-question
    score spread clears an explicit numerical floor.  Homogeneous successes
    never receive an outcome-policy update.  Consequently, a group with no
    observed preference has an exactly zero advantage instead of allowing a
    learned baseline or numerical noise to manufacture one.
    """
    if exact_correctness.ndim != 1 or gold_answer_scores.ndim != 1:
        raise ValueError("correctness and gold scores must be one-dimensional")
    if exact_correctness.shape != gold_answer_scores.shape:
        raise ValueError("correctness and gold scores must have identical shapes")
    group_size = _validate_positive_integer(group_size, "group_size")
    if exact_correctness.numel() == 0 or exact_correctness.numel() % group_size:
        raise ValueError("rollout count must be a positive multiple of group_size")
    if (
        not math.isfinite(float(minimum_gold_score_gap))
        or float(minimum_gold_score_gap) < 0.0
    ):
        raise ValueError("minimum_gold_score_gap must be finite and non-negative")
    ...
    correctness = exact_correctness.detach().float()
    scores = gold_answer_scores.detach().float()
    if not bool(torch.isfinite(correctness).all()):
        raise ValueError("correctness must be finite")
    if not bool(torch.isfinite(scores).all()):
        raise ValueError("gold answer scores must be finite")
    if not bool(((correctness == 0.0) | (correctness == 1.0)).all()):
        raise ValueError("exact correctness must contain only zero or one")

    n_groups = correctness.numel() // group_size
    grouped_correctness = correctness.view(n_groups, group_size)
    grouped_scores = scores.view(n_groups, group_size)
    positive_counts = grouped_correctness.sum(dim=1)
    all_correct = positive_counts == float(group_size)
    all_wrong = positive_counts == 0.0
    mixed = ~(all_correct | all_wrong)
    score_spread = grouped_scores.max(dim=1).values - grouped_scores.min(
        dim=1
    ).values
    likelihood_groups = (
        all_wrong
        & bool(enable_likelihood_fallback)
        & (score_spread >= float(minimum_gold_score_gap))
    )
    informative_groups = mixed | likelihood_groups

    # Average pairwise wins implement a normalized rank with exact tie
    # handling.  The self-comparison contributes the same 0.5 to every path
    # and therefore vanishes under question-local standardization.
    score_differences = grouped_scores.unsqueeze(2) - grouped_scores.unsqueeze(1)
    score_ranks = (
        (score_differences > float(tie_tolerance)).float()
        + 0.5
        * (score_differences.abs() <= float(tie_tolerance)).float()
    ).mean(dim=2)
    grouped_rewards = torch.zeros_like(grouped_scores)
    grouped_rewards = torch.where(
        mixed[:, None],
        grouped_correctness,
        grouped_rewards,
    )
    ...
    standardized = (grouped_rewards - reward_mean) / reward_std.clamp_min(
        float(eps)
    )
    grouped_advantages = torch.where(
        (informative_groups[:, None] & (reward_std >= float(eps))),
        standardized,
        torch.zeros_like(standardized),
    )

    # Calibrate the likelihood proxy only from groups where exact correctness
    # supplies an independent label.  Each correct-vs-wrong pair contributes
    # one, one-half, or zero according to the frozen score ordering.
    correct_left = grouped_correctness.bool().unsqueeze(2)
    wrong_right = (~grouped_correctness.bool()).unsqueeze(1)
    proxy_pairs = mixed[:, None, None] & correct_left & wrong_right
    pair_credit = (
        (score_differences > float(tie_tolerance)).float()
        + 0.5
        * (score_differences.abs() <= float(tie_tolerance)).float()
    )
    proxy_pair_credit = (pair_credit * proxy_pairs.float()).sum()
    proxy_pair_count = proxy_pairs.float().sum()

    return EvidenceGatedGroupOutput(
        policy_rewards=grouped_rewards.reshape_as(correctness),
        path_advantages=grouped_advantages.reshape_as(correctness),
        path_informative_mask=(
            informative_groups[:, None]
            .expand(-1, group_size)
            .reshape_as(correctness)
        ),
        failure_entropy_mask=(
            all_wrong[:, None].expand(-1, group_size).reshape_as(correctness)
        ),
        all_correct_group_mask=all_correct,
        mixed_group_mask=mixed,
        all_wrong_group_mask=all_wrong,
        likelihood_group_mask=likelihood_groups,
        gold_score_spread=score_spread,
        proxy_pair_credit=proxy_pair_credit,
        proxy_pair_count=proxy_pair_count,
    )
