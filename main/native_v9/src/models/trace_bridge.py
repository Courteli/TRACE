...
class LitTRACEBridge(LitREADCoTStableEfficient):
    ...
    Optional Stage 2 uses reproducible multi-view latent rollouts and a GRPO
    answer-token objective with TRACE path rewards.
    """
    ...
    def __init__(self, model_kwargs, training_kwargs, all_config=None):
        ...
        # First-step diagnostics are intentionally limited to global_step=0.
        # They make a genuinely stuck four-rank launch distinguishable from a
        # slow but healthy multi-objective step without changing the objective.
        self._trace_first_step_timing_start = None
        self._trace_first_step_timing_last = None

    def _trace_first_step_mark(self, label: str):
        if int(getattr(self, "global_step", 0)) != 0:
            return
        if torch.cuda.is_available() and self.device.type == "cuda":
            torch.cuda.synchronize(self.device)
        now = time.perf_counter()
        if self._trace_first_step_timing_start is None:
            self._trace_first_step_timing_start = now
            self._trace_first_step_timing_last = now
        elapsed = now - self._trace_first_step_timing_start
        delta = now - self._trace_first_step_timing_last
        self._trace_first_step_timing_last = now
        print(
            f"TRACE_STAGE_TIMING rank={int(getattr(self, 'global_rank', -1))} "
            f"label={label} delta_s={delta:.3f} elapsed_s={elapsed:.3f}",
            flush=True,
        )

    def on_before_backward(self, loss):
        self._trace_first_step_mark("before_backward")
        return super().on_before_backward(loss)

    def on_after_backward(self):
        self._trace_first_step_mark("after_backward")
        return super().on_after_backward()

    def on_before_optimizer_step(self, optimizer):
        self._trace_first_step_mark("before_optimizer_step")
    ...
    def _trace_multiview_losses(
        ...
        view_logs["trace_stage1_multiview_loss"] = (
            view_logs["trace_stage1_view_path_loss"]
    ...
    def _apply_trace_progress_anchors(
        ...
        for sample_idx, item in enumerate(explicit_features):
            ...
            # Keep the distributed logging schema identical on every rank.
            # A one-step rationale has a well-defined progress center/span of
            # zero; omitting these metrics made sync_dist collectives diverge
            # whenever only some ranks received one-step examples.
            ...
            centers.append(
                (assignment.float() * step_pos.unsqueeze(0)).sum(dim=-1)
            )
    ...
    def forward(self, batch):
        self._trace_first_step_mark("forward_start")
        ...
        self._trace_first_step_mark("explicit_teacher_done")
        ...
        self._trace_first_step_mark("compression_done")
        ...
        self._trace_first_step_mark("primary_latents_done")
        ...
        self._trace_first_step_mark("residual_alignment_done")
        ...
        self._trace_first_step_mark("answer_teacher_force_done")
        ...
        self._trace_first_step_mark("anchor_teacher_force_done")
        ...
        self._trace_first_step_mark("path_losses_done")
        ...
        self._trace_first_step_mark("multiview_done")

        # Native role models extend the proven BRIDGE objective through this
        # narrow hook.  Keeping it here avoids reimplementing or bypassing the
        # exact 72%-line answer/anchor/path training code.
        role_logs = {}
        role_hook = getattr(self, "_native_role_training_losses", None)
        if callable(role_hook):
            role_logs = role_hook(
                explicit_features=explicit_features,
                compression_outputs=compression_outputs,
                latent_outputs=latent_outputs,
            )
            if "role_total_loss" not in role_logs:
                raise RuntimeError("native role hook must return role_total_loss")
        self._trace_first_step_mark("role_losses_done")
        ...
        if role_logs:
            total_loss = total_loss + role_logs["role_total_loss"]
        ...
        result.update(role_logs)
        self._trace_first_step_mark("forward_done")
        return result
    ...
    def _trace_rl_exp_batch_size(self) -> int:
        return max(1, int(self.trace_rl_config.get("exp_batch_size", 1)))
