...
SESSION="origin_colar_qwen3_onelatent_r5_cotinit_50epoch_gpu4"
...
tmux new-session -d -s "${SESSION}" \
  "cd '${ROOT}' && source /home/dingxukai/miniconda3/etc/profile.d/conda.sh && conda activate ROT && CUDA_VISIBLE_DEVICES=4 python run.py --model=colar_qwen3_onelatent --dataset=gsm8k_aug_nl --devices=0 --load_ckpt_path='${COT_CKPT}' --do_test --test_times=5 --workspace_path=/home/dingxukai --log_suffix=origin_colar_qwen3_onelatent_r5_cotinit_rawgsm_lr3e-5_50epoch_gpu4 batch_size=1 val_batch_size=1 max_epochs=50 num_sanity_val_steps=0 max_compression_factor=5 compression_factor=5 max_new_tokens=16 lr=3e-5 trainer.default_root_dir='${ROOT}/run_roots/origin_colar_qwen3_onelatent_r5_cotinit_50epoch_gpu4' 2>&1 | tee -a '${RUN_OUTPUTS}/origin_colar_qwen3_onelatent_r5_cotinit_50epoch_gpu4.log'"
