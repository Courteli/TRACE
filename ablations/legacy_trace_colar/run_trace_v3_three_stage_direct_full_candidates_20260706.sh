...
GPU="${GPU:-7}"
ARTIFACT_DIR="${ARTIFACT_DIR:-${ROOT}/run_outputs/trace_multipath/tune_three_stage_20260706/direct_full}"
TARGET_ACC="${TARGET_ACC:-0.30}"
TEST_TIMES="${TEST_TIMES:-1}"
mkdir -p "${ARTIFACT_DIR}"

SUMMARY="${ARTIFACT_DIR}/direct_full_summary.csv"
...
run_full_eval() {
  ...
  mkdir -p "${log_dir}"
  echo "[direct-full] start tag=${tag} max_l=${max_l} min_l=${min_l} latent_temp=${latent_temp} eol_temp=${eol_temp} cf=${compression_factor}"
  ...
  if [[ -z "${json_path}" ]]; then
    echo "[direct-full] ERROR: no test json for ${tag}" >&2
    exit 3
  fi
...
if run_full_eval "full_m8_t1_e1_cf5" 40 8 1.0 1.0 5 16; then
  echo "[direct-full] target reached at full_m8_t1_e1_cf5"
  exit 0
fi
if run_full_eval "full_m12_t1_e1_cf5" 40 12 1.0 1.0 5 16; then
  echo "[direct-full] target reached at full_m12_t1_e1_cf5"
  exit 0
fi
if run_full_eval "full_m16_t1_e1_cf5" 40 16 1.0 1.0 5 16; then
  echo "[direct-full] target reached at full_m16_t1_e1_cf5"
  exit 0
fi
if run_full_eval "full_m12_t07_e1_cf5" 40 12 0.7 1.0 5 16; then
  echo "[direct-full] target reached at full_m12_t07_e1_cf5"
  exit 0
fi

echo "[direct-full] target not reached by direct candidates. Summary: ${SUMMARY}"
