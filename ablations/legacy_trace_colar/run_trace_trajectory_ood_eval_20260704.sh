...
RUN_CONTAINS="${1:-trace_trajectory_full_qwen3_c5_k8}"
GPU="${GPU:-7}"
...
echo "[TRACE trajectory OOD] checkpoint: ${CKPT}"
...
for DATASET in gsm8k_aug_nl gsmhard svamp multiarith; do
  ...
  echo "[TRACE trajectory OOD] dataset=${DATASET}"
