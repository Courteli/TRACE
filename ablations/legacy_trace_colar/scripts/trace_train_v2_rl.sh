...
GPU="${1:-0}"
SFT_CKPT="${2:?Usage: bash scripts/trace_train_v2_rl.sh <gpu> <sft_ckpt>}"
ROOT="/disk1/dingxukai/trace_colar"
RUN_OUTPUTS="${ROOT}/run_outputs/trace"
SESSION="${SESSION_NAME:-trace_v2_rl_qwen3_c5_gpu${GPU}}"
...
TRACE_CENTER_TRAJECTORIES="${TRACE_CENTER_TRAJECTORIES:-False}"
...
tmux new-session -d -s "${SESSION}" \
  "cd '${ROOT}' && \
   ...
   conda activate ROT && \
     ...
     pos_neg_weight=1.0 \
     hard_neg_weight=1.0 \
     pos_pos_diversity_weight=0.2 \
     pos_agreement_weight=0.1 \
	     trace_center_trajectories=${TRACE_CENTER_TRAJECTORIES} \
