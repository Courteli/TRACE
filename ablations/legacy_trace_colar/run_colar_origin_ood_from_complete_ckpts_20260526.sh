...
OUT_DIR="$ROOT/run_outputs/origin_colar_ood_20260526"
mkdir -p "$OUT_DIR"
...
run_eval() {
  ...
  local test_file="$5"
  local gpu="$6"
  local version="20260526_origin_colar_r${factor}_${dataset}_ood"
...
run_group() {
  ...
  local ckpt="$2"
  local gpu="$3"
  run_eval "$factor" "$ckpt" gsmhard "/home/dingxukai/RoT/data/GSM8k-Hard" "gsmhard_test_processed.jsonl" "$gpu"
...
echo "[$(date '+%F %T')] ALL OOD EVALS DONE" | tee -a "$OUT_DIR/summary.log"
